...
class _ProfileViewState extends State<ProfileView> {
  ...
  final AsyncMemoizer<User> _memoizer = AsyncMemoizer();

  Future<User> _getUser() {
    return this._memoizer.runOnce(() async {
      User user = await AppFirestore.getUser(widget.username);
      return user ;
    });
  }
          ...
          return  Scaffold(
            appBar: AppBar(
              ...
              actions: [
                SizedBox(
                  width: 35,
                  child: IconButton(icon:Icon(Icons.add , color: Colors.orangeAccent[300] ),
                    onPressed: () async {
                      Navigator.push(context, MaterialPageRoute(builder: (context) => NewPost(username: widget.username)));
                    },
            ...
            body: RefreshIndicator(
              onRefresh: () async {
                user = await AppFirestore.getUser(widget.username);
              },
